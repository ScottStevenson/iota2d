...
public class Game
{
	...
	protected void loadLevel( String ID )
	{
		...
		try {
			ship = entityFactory.buildShip( database.get( "levels." + ID, "player" ));
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		level = levelFactory.buildLevel( ID, ship );
		if( Boolean.parseBoolean( database.get( "levels." + ID + ".HUD", "hasHUD" )))
		{
			level.addHud( new HUD( ship ));
		}
		LinkedList enemies = level.getEntities( "enemy" );
		for( int i = 0; i < enemies.size(); i++ )
		{
			Enemy enemy = (Enemy) enemies.get(i);
			AlienAI ai = new AlienAI( enemy.coll );
			enemy.addAI( ai );
		}
	}
